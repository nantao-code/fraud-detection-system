...
from feature_engineering import CategoricalConverter
...
class PipelineTraining:
    ...
    def _detect_feature_types(self, X: pd.DataFrame) -> Tuple[List[str], List[str]]:
        ...
        # 从配置获取阈值
        unique_value_threshold = self.config.get('feature_engineering.unique_value_threshold', 10)
        
        for col in X.columns:
            if X[col].dtype in ['int64', 'float64']:
                # 如果唯一值较少，可能是类别特征
                if X[col].nunique() <= unique_value_threshold:
    ...
    def _create_preprocessor(self, numeric_features: List[str], 
                           ...
                           model_name: str=None) -> ColumnTransformer:
        """
        ...
        if model_type_upper == 'LR':
            ...
            logging.info(f"为{model_type}模型使用StandardScaler标准化数值特征")
            ...
            logging.info(f"为{model_type}模型不对数值特征进行缩放处理")
            ...
            logging.info(f"为{model_type}模型不对数值特征进行缺失值填充与缩放处理")
            ...
            logging.info(f"为{model_type}模型使用专为XGBoost/LightGBM优化的CategoricalConverter")
    ...
    def _create_pipeline(self, numeric_features: List[str], 
        ...
        # 3. 分类器（使用ModelFactory创建）
        model, param_grid = self._create_model(model_type)
        steps.append(('classifier', model))
        
        # 根据是否使用过采样选择Pipeline类型
        if imbalance_method != 'none':
            pipeline = ImbPipeline(steps=steps)
        else:
            pipeline = Pipeline(steps=steps)
        
        # 直接返回不带前缀的param_grid，前缀处理移到train_model中
        return pipeline, param_grid
    
    def _feature_engineering_pipeline(self, X_train: pd.DataFrame, y_train: pd.Series, X_test: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame, List[str]]:
        """
        执行特征工程处理（使用新的scikit-learn兼容管道）
        ...
        Returns:
            (处理后的训练特征, 处理后的测试特征, 最终特征列表)
        """
        try:
            from feature_engineering import FeatureEngineeringPipeline
            feature_engineering = FeatureEngineeringPipeline(self.config)
            
            # 使用新的scikit-learn兼容方法
            feature_engineering.fit(X_train, y_train)
            X_train_processed = feature_engineering.transform(X_train)
            X_test_processed = feature_engineering.transform(X_test)
            
            # 获取最终特征列表
            final_features = feature_engineering.get_feature_names_out()
            
            logging.info(f"特征工程完成，最终特征数量: {len(final_features)}")
            
            return X_train_processed, X_test_processed, final_features
            
        except Exception as e:
            logging.warning(f"特征工程执行失败，使用原始特征: {e}")
    ...
    def train_model(self, 
        ...
        numeric_features, categorical_features = self._detect_feature_types(X_train)
        logging.info(f"检测到 {len(numeric_features)} 个数值特征和 {len(categorical_features)} 个类别特征。")

        pipeline, param_grid = self._create_pipeline(numeric_features, categorical_features, model_type)
    ...
    def run_single_model(self, model_name: str = None) -> Dict[str, Any]:
        ...
        try:
            ...
            # 5. 特征工程 - 先对原始训练集做特征工程
            logging.info("步骤5/8: 开始特征工程处理...")
            X_train_processed, X_test_processed, final_features = self._feature_engineering_pipeline(
                X_train, y_train, X_test
            )
            logging.info(f"特征工程完成，最终特征数量: {len(final_features)}")
            logging.info(f"最终特征: {final_features[:5]}...")

            # 6. 训练模型
            logging.info("步骤6/8: 开始模型训练...")
            ...
            try:
                pipeline, evaluation_results, best_params = self.train_model(
    ...
    def _save_pipeline(self, pipeline: Pipeline, model_name: str, results: Dict[str, Any] = None) -> str:
        ...
        pmml_filename = self.config.get(
            ...
            'pmmls/puts/pipeline_{input_data}_{imbalance_method}_{model_name}.pmml'
        ).format(
            input_data=input_data_name,
            imbalance_method=imbalance_method,
            model_name=model_name
        ...
        try:
            ...
            # 检查传入的pipeline是否包含采样步骤
            if isinstance(pipeline, ImbPipeline):
                logging.info("检测到ImbPipeline，正在创建用于PMML的纯净预测管道...")
                
                # 提取除了'sampler'之外的所有步骤
                prediction_steps = []
                for step_name, step_transformer in pipeline.steps:
                    if step_name != 'sampler':
                        prediction_steps.append((step_name, step_transformer))
                
                # 创建一个新的、标准的scikit-learn Pipeline
                pmml_pipeline = SklearnPipeline(steps=prediction_steps)
                
                logging.info("纯净预测管道创建完成，包含步骤: " + str([name for name, _ in pmml_pipeline.steps]))
            else:
                # 如果不是ImbPipeline，直接使用原始pipeline
                pmml_pipeline = pipeline

            print(f"正在将{model_name} Pipeline保存到: {pmml_path}")
            sklearn2pmml(pmml_pipeline, pmml_path, with_repr=True) # <--- 使用新的 pmml_pipeline
            print(f"{model_name} PMML文件已生成,路径为{pmml_path}")
